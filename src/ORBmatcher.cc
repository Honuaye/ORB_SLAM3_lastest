...
namespace ORB_SLAM3
{
    ...
    // yhh
    int ORBmatcher::SearchForUpdateDepthFilter(Frame *CurrentFrame, KeyFrame *pKF,
                                           vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse) {
        const DBoW2::FeatureVector &vFeatVec1 = CurrentFrame->mFeatVec;
        const DBoW2::FeatureVector &vFeatVec2 = pKF->mFeatVec;

        //Compute epipole in second image
        Sophus::SE3f T1w = CurrentFrame->GetPose();
        Sophus::SE3f T2w = pKF->GetPose();
        Sophus::SE3f Tw2 = pKF->GetPoseInverse(); // for convenience
        Eigen::Vector3f Cw = CurrentFrame->GetCameraCenter();
        Eigen::Vector3f C2 = T2w * Cw;

        Eigen::Vector2f ep = pKF->mpCamera->project(C2);
        Sophus::SE3f T12;
        Sophus::SE3f Tll, Tlr, Trl, Trr;
        Eigen::Matrix3f R12; // for fastest computation
        Eigen::Vector3f t12; // for fastest computation

        GeometricCamera* pCamera1 = CurrentFrame->mpCamera, *pCamera2 = pKF->mpCamera;
        ...
        // Compare only ORB that share the same node
        int nmatches=0;
        vector<bool> vbMatched2(pKF->N,false);
        vector<int> vMatches12(CurrentFrame->N,-1);
        ...
        while(f1it!=f1end && f2it!=f2end)
        {
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    MapPoint* pMP1 = CurrentFrame->GetMapPoint(idx1);
                    ...
                    const bool bStereo1 = (!CurrentFrame->mpCamera2 && CurrentFrame->mvuRight[idx1]>=0);

                    if(bOnlyStereo)
                        if(!bStereo1)
                            continue;

                    // const cv::KeyPoint &kp1 = (CurrentFrame -> NLeft == -1) ? CurrentFrame->mvKeysUn[idx1]
                    //                                                 : (idx1 < CurrentFrame -> NLeft) ? CurrentFrame -> mvKeys[idx1]
                    //                                                                          : CurrentFrame -> mvKeysRight[idx1 - CurrentFrame -> NLeft];
                    // const bool bRight1 = (CurrentFrame -> NLeft == -1 || idx1 < CurrentFrame -> NLeft) ? false
                    //                                                                    : true;
                    const bool bRight1 = false;
                    const cv::KeyPoint &kp1 = CurrentFrame->mvKeysUn[idx1];
                    ...
                    const cv::Mat &d1 = CurrentFrame->mDescriptors.row(idx1);
                    ...
                    for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                    {
                        ...
                        MapPoint* pMP2 = pKF->GetMapPoint(idx2);

                        // If we have already matched or there is a MapPoint skip
                        if(vbMatched2[idx2] || pMP2)
                            continue;

                        const bool bStereo2 = (!pKF->mpCamera2 &&  pKF->mvuRight[idx2]>=0);

                        if(bOnlyStereo)
                            if(!bStereo2)
                                continue;

                        const cv::Mat &d2 = pKF->mDescriptors.row(idx2);
                        ...
                        const cv::KeyPoint &kp2 = (pKF -> NLeft == -1) ? pKF->mvKeysUn[idx2]
                                                                        : (idx2 < pKF -> NLeft) ? pKF -> mvKeys[idx2]
                                                                                                 : pKF -> mvKeysRight[idx2 - pKF -> NLeft];
                        const bool bRight2 = (pKF -> NLeft == -1 || idx2 < pKF -> NLeft) ? false
                                                                                           : true;

                        if(!bStereo1 && !bStereo2 && !CurrentFrame->mpCamera2)
                        {
                            const float distex = ep(0)-kp2.pt.x;
                            const float distey = ep(1)-kp2.pt.y;
                            if(distex*distex+distey*distey<100*pKF->mvScaleFactors[kp2.octave])
                            {
                                continue;
                            }
                        }
                        ...
                        if(bCoarse || pCamera1->epipolarConstrain(pCamera2,kp1,kp2,R12,t12,CurrentFrame->mvLevelSigma2[kp1.octave],pKF->mvLevelSigma2[kp2.octave])) // MODIFICATION_2
                        {
                            bestIdx2 = idx2;
                            bestDist = dist;
                        }
                    ...
                    if(bestIdx2>=0)
                    {
                        const cv::KeyPoint &kp2 = (pKF -> NLeft == -1) ? pKF->mvKeysUn[bestIdx2]
                                                                        : (bestIdx2 < pKF -> NLeft) ? pKF -> mvKeys[bestIdx2]
                                                                                                     : pKF -> mvKeysRight[bestIdx2 - pKF -> NLeft];
                        vMatches12[idx1]=bestIdx2;
                        nmatches++;
